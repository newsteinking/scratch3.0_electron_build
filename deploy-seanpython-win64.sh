...
cd editor
mkdir python-ide
git clone https://github.com/newsteinking/python-code-editor python-ide-source
cd python-ide-source
#npm install > /dev/null 2>&1
#npm run build > /dev/null 2>&1
cp -r * ../
cd ../
rm -r -f "python-ide-source"

cd ../
echo "Win x64"
./node_modules/.bin/electron-packager . "PythonIDE3.0" --asar --platform win32 --arch x64 --out out --icon seanlablogo.ico --prune false
#echo "Win x32"
...
# deploy
cd out
zip -r -q -9 PythonIDE3.0_Win32-x64.zip "PythonIDE3.0-win32-x64"
rm -r "PythonIDE3.0-win32-x64"
...
cp ../index.md ./
cp ../_config.yml ./
cp ../README-out-python.md ./README.md
git init
git lfs install
git lfs track *
git add .
...
git push --force "https://github.com/newsteinking/python-code-editor" master:gh-pages
exit 0
